...
def encode_db_url(url: str) -> str:
    ...
    # Check if it's already encoded to prevent double encoding
    if "%" not in password:
...
def sync_database():
    ...
    with local_engine.connect() as local_conn, remote_engine.connect() as remote_conn:
        # Reflect table structures from both databases
        ...
        for table_name in tables_to_sync:
            ...
            # Support both SQLAlchemy 1.x and 2.x cursor mappings
            ...
            if not pk_cols:
                # Fallback to pure inserts if no primary key is found to do an upsert
