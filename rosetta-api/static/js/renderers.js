...
function clearReport() {
    ...
    Object.entries(panelElements).forEach(([key, panel]) => {
        if (key !== 'raw') {
            if (panel) panel.innerHTML = ''; // Add null check
        }
        if (panel) panel.classList.add('hidden'); // Add null check
    });
    if (jsonOutput) jsonOutput.textContent = ''; // Add null check
}

function showTab(tabName) {
    welcomeMessage.classList.add('hidden');
    Object.values(panelElements).forEach(panel => { if (panel) panel.classList.add('hidden') }); // Add null check
...
function renderSuiteSummary(data, title) {
    ...
    html += `<div class="flex justify-start items-center space-x-4 text-xs text-gray-600 mb-4 p-2 bg-gray-50 rounded-lg border">
                ...
                <span class="flex items-center"><span class="w-3 h-3 inline-block rounded-full bg-green-100 border border-green-300 mr-1.5"></span>Success / No Divergences</span>
                <span class="flex items-center"><span class="w-3 h-3 inline-block rounded-full bg-yellow-100 border border-yellow-300 mr-1.5"></span>Divergences Found</span>
                <span class="flex items-center"><span class="w-3 h-3 inline-block rounded-full bg-gray-100 border border-gray-300 mr-1.5"></span>Offline</span>
                <span class="flex items-center"><span class="w-3 h-3 inline-block rounded-full bg-red-100 border border-red-300 mr-1.5"></span>Anomaly</span>
            </div>`;
    // --- END LEGEND ---
        ...
        summary.forEach((item, idx) => {
            ...
            if (item.error) {
                if (isConnectionError(item.error)) {
                    status = `<span class="px-2 py-1 text-xs rounded bg-gray-100 text-gray-600">Offline</span>`;
                } else {
                    status = `<span class="px-2 py-1 text-xs rounded bg-red-100 text-red-800">Anomaly</span>`;
                }
            }
...
function renderRankingTable(report, key, unit, panel) {
    if (!panel) return; // Add null check
    let html = `<h3 class="font-semibold text-gray-900 mt-4 mb-2">${report.summary}</h3>`;

    // --- ADDED LEGEND ---
    html += `<div class="flex justify-start items-center space-x-4 text-xs text-gray-600 mb-4 p-2 bg-gray-50 rounded-lg border">
                <span class="font-bold">Legend:</span>
                <span class="flex items-center"><span class="w-3 h-3 inline-block rounded-full bg-white border border-gray-300 mr-1.5"></span>Success</span>
                <span class="flex items-center"><span class="w-3 h-3 inline-block rounded-full bg-gray-100 border border-gray-400 mr-1.5"></span>Offline</span>
                <span class="flex items-center"><span class="w-3 h-3 inline-block rounded-full bg-red-100 border border-red-300 mr-1.5"></span>Anomaly</span>
            </div>`;
    // --- END LEGEND ---

    // Get offline simulators
    const offlineSims = new Set();
    if (currentSuiteData && currentSuiteData.raw_results) {
        currentSuiteData.raw_results.forEach(r => {
            if (r.error && isConnectionError(r.error)) {
                offlineSims.add(r.simulator);
            }
        });
    }
    if (key === 'memory_usage_mb') {
        ...
        html += `<div class="overflow-x-auto border rounded-lg"><table class="min-w-full divide-y divide-gray-200"><thead class="bg-gray-50"><tr>
                    <th class="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase" style="cursor: help; text-decoration: underline dotted;" title="The name of the runner service.">Simulator</th>
                    <th class="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase" style="cursor: help; text-decoration: underline dotted;" title="Actual RAM added by the circuit execution (End RSS - Start RSS). Can be 0 for small circuits using pre-allocated memory.">Mem Delta (MiB)</th>
                    <th class="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase" style="cursor: help; text-decoration: underline dotted;" title="The absolute maximum RAM used by the runner container during execution.">Process Peak (MiB)</th>
                </tr></thead><tbody class="bg-white divide-y divide-gray-200">`;
        ...
        report.ranking.forEach(r => {
            ...
            if (r.status === 'success') {
                ...
                mem_delta_display = `<span class="text-gray-400 italic">Offline</span>`;
                proc_peak_display = `<span class="text-gray-400 italic">Offline</span>`;
            }
        ...
        html += `<div class="overflow-x-auto border rounded-lg"><table class="min-w-full divide-y divide-gray-200"><thead class="bg-gray-50"><tr>
                    ...
                    <th class="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase" style="cursor: help; text-decoration: underline dotted;" title="Time spent transpiling and optimizing the circuit">Compilation (s)</th>
                    <th class="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase" style="cursor: help; text-decoration: underline dotted;" title="Time spent performing the actual quantum state evolution">Simulation (s)</th>
                    <th class="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase" style="cursor: help; text-decoration: underline dotted;" title="End-to-end execution time">Total (s)</th>
                </tr></thead><tbody class="bg-white divide-y divide-gray-200">`;
        ...
        report.ranking.forEach((r, i) => {
            ...
            if (r.status === 'success') {
                ...
                total_display = `<span class="text-gray-400 italic">Offline</span>`;
            } else {
        ...
        report.ranking.forEach((r, i) => {
            ...
            if (r.status === 'success' && r[key] !== null) {
                ...
                val_display = `<span class="text-gray-400 italic">Offline</span>`;
            }
...
function renderDivergenceTables(report, panel) {
    ...
    html += `<div class="flex justify-start items-center space-x-4 text-xs text-gray-600 mb-4 p-2 bg-gray-50 rounded-lg border">
                <span class="font-bold">Legend:</span>
                <span class="flex items-center"><span class="w-3 h-3 inline-block rounded-full bg-green-50 border border-green-300 mr-1.5"></span>Perfect Match</span>
                <span class="flex items-center"><span class="w-3 h-3 inline-block rounded-full bg-yellow-50 border border-yellow-300 mr-1.5"></span>Divergence</span>
                <span class="flex items-center"><span class="w-3 h-3 inline-block rounded-full bg-gray-200 border border-gray-400 mr-1.5"></span>Offline</span>
                <span class="flex items-center"><span class="w-3 h-3 inline-block rounded-full bg-red-50 border border-red-300 mr-1.5"></span>Anomaly</span>
        ...
        report.simulators.forEach((s1, i) => {
            ...
            report[key][i].forEach((val, j) => {
                ...
                if (offlineSims.has(s1) || offlineSims.has(report.simulators[j])) {
                    txt = "OFF";
                    bg = 'bg-gray-200 text-gray-500 italic';
                } else {
                    const epsilon = 1e-9;
                    ...
                    if (isPerfectMatch) {
                        ...
                        txt = "ERR";
                        bg = 'bg-red-50 text-red-700 font-bold';
                    }
                }
