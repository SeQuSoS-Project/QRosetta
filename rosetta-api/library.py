...
# Using standard print or python logging if get_logger isn't available
try:
    from qrosetta_commons.helpers import get_logger
    logger = get_logger("rosetta-library")
except ImportError:
    import logging
    logger = logging.getLogger("rosetta-library")

# Cache for loaded algorithms mapping id to metadata/code
_ALGORITHMS_CACHE: Dict[str, Any] = {}

def load_algorithms() -> Dict[str, Any]:
    """Loads algorithm JSON files from data/algorithms/ directory."""
...
def generate_circuit(algorithm_id: str, n_qubits: int) -> str:
    ...
    try:
        exec(algo_data["code"], globals(), local_context)
