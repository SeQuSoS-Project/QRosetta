...
"""
Local smoke test for the qrisp runner.

Run against the live stack:
    python3 tests/test_qrisp_runner.py
"""
import httpx
import time
...
def test_statevector():
    ...
    task = summary[0]
    qrisp_result = next(
        (r for r in task.get("raw_results", []) if r.get("simulator") == "qrisp"),
        None
    )
    assert qrisp_result is not None, "No result for qrisp in raw_results"
    assert qrisp_result.get("error") is None, f"qrisp runner error: {qrisp_result.get('error')}"
    print(f"  execution_time_sec  : {qrisp_result.get('execution_time_sec'):.4f}s")
    print(f"  compilation_time_sec: {qrisp_result.get('compilation_time_sec'):.4f}s")
    print(f"  simulation_time_sec : {qrisp_result.get('simulation_time_sec'):.4f}s")
    print(f"  memory_usage_mb     : {qrisp_result.get('memory_usage_mb'):.2f} MB")
    print("  PASSED")
...
def test_measured():
    ...
    assert qrisp_result is not None, "No result for qrisp in raw_results"
    assert qrisp_result.get("error") is None, f"qrisp runner error: {qrisp_result.get('error')}"
    counts = qrisp_result.get("counts", {})
