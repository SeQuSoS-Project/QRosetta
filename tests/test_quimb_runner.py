...
"""
Local smoke test for the Quimb runner.

Run against the live stack:
    python3 tests/test_quimb_runner.py
"""
import httpx
import time
...
def test_statevector():
    ...
    task = summary[0]
    result = next(
        (r for r in task.get("raw_results", []) if r.get("simulator") == "quimb"),
        None
    )
    assert result is not None, "No result for quimb in raw_results"
    assert result.get("error") is None, f"quimb runner error: {result.get('error')}"
    print(f"  execution_time_sec  : {result.get('execution_time_sec'):.4f}s")
    print(f"  compilation_time_sec: {result.get('compilation_time_sec'):.4f}s")
    print(f"  simulation_time_sec : {result.get('simulation_time_sec'):.4f}s")
    print(f"  memory_usage_mb     : {result.get('memory_usage_mb'):.2f} MB")
    print("  PASSED")
