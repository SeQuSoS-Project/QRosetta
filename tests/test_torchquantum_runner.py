...
Run against the live stack:
    python3 tests/test_torchquantum_runner.py
"""
import httpx
import time
...
def test_statevector():
    ...
    payload = {
        ...
        "mode": "statevector",
        "target_simulators": ["torchquantum"]
    }
    ...
    task = summary[0]
    tq_result = next(
        (r for r in task.get("raw_results", []) if r.get("simulator") == "torchquantum"),
        None
    )
    assert tq_result is not None, "No result for torchquantum in raw_results"
    assert tq_result.get("error") is None, f"torchquantum runner error: {tq_result.get('error')}"
    print(f"  execution_time_sec  : {tq_result.get('execution_time_sec'):.4f}s")
    print(f"  compilation_time_sec: {tq_result.get('compilation_time_sec'):.4f}s")
...
if __name__ == "__main__":
    try:
        test_statevector()
        test_measured()
        print("\nAll TorchQuantum smoke tests PASSED.")
