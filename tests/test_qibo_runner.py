"""
Local smoke test for the qibo runner.

Run against the live stack:
    python3 tests/test_qibo_runner.py
"""
import httpx
import time
...
def test_statevector():
    ...
    task = summary[0]
    qibo_result = next(
        (r for r in task.get("raw_results", []) if r.get("simulator") == "qibo"),
        None
    )
    assert qibo_result is not None, "No result for qibo in raw_results"
    assert qibo_result.get("error") is None, f"qibo runner error: {qibo_result.get('error')}"
    print(f"  execution_time_sec  : {qibo_result.get('execution_time_sec'):.4f}s")
    print(f"  compilation_time_sec: {qibo_result.get('compilation_time_sec'):.4f}s")
    print(f"  simulation_time_sec : {qibo_result.get('simulation_time_sec'):.4f}s")
    print(f"  memory_usage_mb     : {qibo_result.get('memory_usage_mb'):.2f} MB")
    print("  PASSED")
