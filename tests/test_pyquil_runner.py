...
"""
Local smoke test for the pyquil runner.

Run against the live stack:
    python3 tests/test_pyquil_runner.py
"""
import httpx
import time
...
def test_statevector():
    ...
    task = summary[0]
    pyquil_result = next(
        (r for r in task.get("raw_results", []) if r.get("simulator") == "pyquil"),
        None
    )
    assert pyquil_result is not None, "No result for pyquil in raw_results"
    assert pyquil_result.get("error") is None, f"pyquil runner error: {pyquil_result.get('error')}"
    print(f"  execution_time_sec  : {pyquil_result.get('execution_time_sec'):.4f}s")
    print(f"  compilation_time_sec: {pyquil_result.get('compilation_time_sec'):.4f}s")
    print(f"  simulation_time_sec : {pyquil_result.get('simulation_time_sec'):.4f}s")
    print(f"  memory_usage_mb     : {pyquil_result.get('memory_usage_mb'):.2f} MB")
    print("  PASSED")
...
def test_measured():
    ...
    assert pyquil_result is not None, "No result for pyquil in raw_results"
    assert pyquil_result.get("error") is None, f"pyquil runner error: {pyquil_result.get('error')}"
    counts = pyquil_result.get("counts", {})
